...
class Log(@volatile var dir: File,
  ...
  locally {
    ...
    // 初始化Leader Epoch Cache ==> 1. 创建Leader Epoch检查点文件 2. 生成Leader Epoch Cache对象
    initializeLeaderEpochCache()
  ...
  private def loadSegments(): Long = {
    ...
    // 它会移除上次Failure遗留下来的各种临时文件（包括.cleaned、.swap、.deleted文件等）
    ...
    retryOnOffsetOverflow {
      ...
      // 它会清空所有日志段对象，并且再次遍历分区路径，重建日志段segments Map以及索引文件。
